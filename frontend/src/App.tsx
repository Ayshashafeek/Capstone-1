...
function Pipeline() {
  ...
  return <div className="pipeline"><div className="content-heading"><div><p className="eyebrow">03 / FUNDING PIPELINE</p><h1>Keep the next move visible.</h1><p className="muted">Your saved opportunities, current status, and upcoming deadlines in one place.</p></div></div>{summary && <div className="summary-grid"><div><strong>{summary.saved_count}</strong><span>saved leads</span></div><div><strong>{summary.active_count}</strong><span>active reviews</span></div><div><strong>{summary.applied_count}</strong><span>applications</span></div><div><strong>${(summary.pipeline_amount_cents / 100).toLocaleString()}</strong><span>pipeline ceiling</span></div></div>}{error && <p className="error">{error}</p>}{loading ? <div className="catalogue-empty">Loading pipeline...</div> : items.length === 0 ? <div className="catalogue-empty"><strong>Your pipeline is empty.</strong><span>Save a lead from the catalogue to start tracking it.</span></div> : <div className="pipeline-list">{items.map(item => <PipelineRow key={item.id} item={item} onUpdate={update} onDelete={remove} />)}</div>}</div>
}
...
function PipelineRow({ item, onUpdate, onDelete }: { item: SavedGrant; onUpdate: (item: SavedGrant, status: string, notes: string) => void; onDelete: (id: string) => void }) {
  ...
  return <article className="pipeline-row"><div><span className="source-badge">{item.status.toUpperCase()}</span><h2>{item.grant.title}</h2><p className="grant-funder">{item.grant.funder} · Deadline {item.grant.deadline ? new Date(item.grant.deadline).toLocaleDateString() : 'not listed'}</p><p className="pipeline-reason">{item.grant.match_score}% match · {item.grant.match_reasons.join(' · ') || 'Review eligibility details'}</p></div><div className="pipeline-controls"><label>Status<select value={item.status} onChange={event => onUpdate(item, event.target.value, notes)}><option value="saved">Saved</option><option value="reviewing">Reviewing</option><option value="applied">Applied</option><option value="rejected">Rejected</option><option value="won">Won</option><option value="archived">Archived</option></select></label><label>Notes<textarea value={notes} onChange={event => setNotes(event.target.value)} onBlur={() => onUpdate(item, item.status, notes)} placeholder="Next action or context..." rows={2} /></label><button className="quiet-button" onClick={() => onDelete(item.id)}>Remove</button></div></article>
}
